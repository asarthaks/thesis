...
# worker.sh lives in scripts/, one level below the repo root. Run from the root so the
# eval'd `python scripts/run_experiment.py ...` commands and relative out_dir/status paths
# resolve identically to the pre-restructuring tree.
