...
# repo-root shim: this entry script lives in scripts/; add the repo root (its parent)
# to sys.path so the promoted packages (core/, Methods/, diagnostics/, revision/) import.
import os, sys
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from Methods.Scripts.cls import langevin_infilling_multiple_positions as legacy_cls_multi
...
def verify_equivalence():
    ...
    mask_indices = [2, 4] # Masking "brown" and "jumps"
    orig_ids = input_ids.clone()
    
    # Hardcode the corrupted tokens
    ...
    legacy_s_ids, legacy_metrics = legacy_cls_multi(
        model=model,
        ...
        debug=False,
        mh_burn_in=0,            # Match the OO code behavior
        noise_scale=noise_scale
    )

    print("\n--- Running New OO CLS Code ---")
    ...
    sampler = ContinuousLangevinSampler(
        model=model, 
        tokenizer=tokenizer, 
        steps=steps, 
        temperature=temperature,
    ...
    if len(legacy_grads) != len(new_grads):
        ...
        for i in range(len(legacy_grads)):
            ...
            # Verify the tensors are mathematically identical
    ...
    if passed:
        print("\n✅ SUCCESS! The refactored OO CLS code is mathematically and physically identical to the legacy code.")
    else:
