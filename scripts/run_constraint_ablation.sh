...
# scripts/ -> repo root, so `python scripts/...` and relative results/logs paths resolve.
cd "$(dirname "$(readlink -f "$0")")/.." || exit 1

OUT=results/constrained/main
mkdir -p "$OUT" logs/constrained
...
import json, glob
rows = {}
for f in glob.glob('results/constrained/main/*.json'):
