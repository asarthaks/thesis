...
# This launcher lives in scripts/, one level below the repo root. Run from the root so
# relative out_dir/status/manifest paths and `python scripts/...` commands resolve.
...
for g in $GPUS; do
  for i in $(seq 1 "$PER_GPU"); do
    ...
    tmux send-keys -t "$SESSION:$win" "cd '$REPO_ROOT' && bash '$REPO_ROOT/scripts/worker.sh' $g $VRAM $MANIFEST $OUTDIR $STATUS" C-m
  done
done
...
if tmux has-session -t "$SESSION" 2>/dev/null; then
  ...
  echo "if a worker window shows nothing, run 'bash $REPO_ROOT/scripts/worker.sh <gpu> $VRAM $MANIFEST $OUTDIR $STATUS' in it by hand to see the error."
else
