...
# scripts/ lives one level below the repo root; run from the root so `python scripts/...`
# and relative log/results paths resolve.
...
# ---- build the job list ---------------------------------------------------
mapfile -t JOBS < <(python scripts/gen_jobs.py "${GEN_ARGS[@]}")
NJOBS=${#JOBS[@]}
NGPU=${#GPUS[@]}
