...
% and identically in the abstract, Section 5.5, Section 6.1, Section 6.2 and Section 7.
...
%
% \subsection{The Shape of the Investigation}
...
% direct measurement and reached by two independent routes. The narrative of the results section
...
\begin{enumerate}
    \item A controlled ablation that separates the direction of the gradient from its magnitude, run across five energy functions, establishing that in the tested token-substitution settings the gradient's direction gave no reliable advantage over a norm-matched random one.
    \item A measurement of the \emph{linearization failure} behind that result. The region over which the sampler's first-order approximation holds is smaller than the smallest admissible token substitution. The gradient is therefore inapplicable rather than merely imprecise.
...
% five compact contributions. Currently it reproduces the complete Results section in
...
% section clause by clause and previewed the results; it is superseded rather than deleted,
